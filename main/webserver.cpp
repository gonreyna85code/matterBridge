...
static std::vector<std::string> index_to_uid;
...
{
    index_to_uid.clear();
    index_to_uid.reserve(g_device_map ? g_device_map->size() : 0);
    ...
    html << "<table><tr>"
         << "<th>ID</th><th>UID</th><th>Endpoint</th><th>IP</th><th>Status</th><th>Last seen</th><th>Settings</th><th>Remove</th>"
         << "</tr>";
    ...
    if (g_device_map && !g_device_map->empty())
    {
        ...
        int idx = 0;
        for (auto &it : *g_device_map)
        {
            index_to_uid.push_back(it.first);
            ...
            for (auto &ep : d.endpoints)
            {
                ...
                html << "<td><a href='/remove?id=" << idx
                     << "&ep=" << t << "' style='color:#f44;font-weight:bold'>✖</a></td>";
    ...
    {
        html << "<tr><td colspan='8'>No devices detected</td></tr>";
    }
...
static esp_err_t handle_remove_endpoint(httpd_req_t *req)
{
    ...
    char id_raw[16], ep[32];
    if (httpd_query_key_value(buf, "id", id_raw, sizeof(id_raw)) != ESP_OK ||
        httpd_query_key_value(buf, "ep", ep, sizeof(ep)) != ESP_OK)
    {
        free(buf);
        return ESP_FAIL;
    }
    ...
    int idx = atoi(id_raw);
    if (idx < 0 || idx >= (int)index_to_uid.size())
        return ESP_FAIL;

    const std::string &uid = index_to_uid[idx];

    devices::remove_endpoint(uid, ep);
